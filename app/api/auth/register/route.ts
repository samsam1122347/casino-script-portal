...
export async function POST(request: Request) {
  ...
  const res = await serverLaravelFetch("/api/v1/auth/register", {
    method: "POST",
  ...
  const payload: { token?: string; message?: string; errors?: unknown } =
    await res.json().catch(() => ({}));

  if (!res.ok) {
    return NextResponse.json(
      { message: payload.message ?? "Registration failed.", errors: payload.errors },
      { status: res.status },
    );
  }
